<x-app-layout>
    <x-slot name="header">Fakultas</x-slot>
    <x-slot name="title">Fakultas</x-slot>

    <x-card>
        <div class="mb-4 flex justify-end">
            <x-link-button href="{{ route('faculties.create') }}">Tambah Fakultas</x-link-button>
        </div>
        ...
        <x-table>
            ...
            <x-slot name="body">
                @forelse($faculties as $faculty)
                    <tr>
                        <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{{ $faculty->code }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{{ $faculty->name }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                            <a href="{{ route('faculties.edit', $faculty) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            <form action="{{ route('faculties.destroy', $faculty) }}" method="POST" class="inline ml-2">
                                @csrf
                                @method('DELETE')
