<x-app-layout>
    <x-slot name="header">Edit Program Studi</x-slot>
    <x-slot name="title">Edit Program Studi</x-slot>

    <x-card>
        <form action="{{ route('study-programs.update', $studyProgram) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="faculty_id" name="faculty_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    ...
                    @foreach($faculties as $faculty)
                        <option value="{{ $faculty->id }}" {{ old('faculty_id', $studyProgram->faculty_id) == $faculty->id ? 'selected' : '' }}>{{ $faculty->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" :value="old('code', $studyProgram->code)" required />
                <x-input-error :messages="$errors->get('code')" class="mt-2" />
            </div>

            <div>
                <x-input-label for="name" value="Nama Prodi" />
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $studyProgram->name)" required />
            ...
            <div>
                ...
                <select id="degree" name="degree" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    @foreach(['D3', 'S1', 'S2', 'S3'] as $degree)
                        <option value="{{ $degree }}" {{ old('degree', $studyProgram->degree) == $degree ? 'selected' : '' }}>{{ $degree }}</option>
                    @endforeach
                </select>
