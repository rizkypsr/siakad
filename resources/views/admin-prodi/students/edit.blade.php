        ...
        <form action="{{ route('students.update', $student) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $student->user->name)" required />
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <x-text-input id="year_of_entry" name="year_of_entry" type="number" class="mt-1 block w-full" :value="old('year_of_entry', $student->year_of_entry)" min="2000" max="2100" required />
                ...
                <div>
                    ...
                    <select id="study_program_id" name="study_program_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        @foreach($studyPrograms as $prodi)
                            <option value="{{ $prodi->id }}" {{ old('study_program_id', $student->study_program_id) == $prodi->id ? 'selected' : '' }}>{{ $prodi->name }}</option>
                        @endforeach
                    </select>
